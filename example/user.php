...
$result = $instagram->searchUser( $user, 1 );
$error_message = '';
if ( !empty( $result->data ) && !empty( $result->data[0]->id ) ) {
  $result = $instagram->getUserMedia( $result->data[0]->id );
  if ( !empty( $result->meta ) && ! empty( $result->meta->error_message ) ) {
    $error_message =  "<h1>Request Failed</h1>" . $result->meta->error_message;
    $result = new stdClass();
    $result->data = [];
  }
...
<html lang="en">
  <head>
    ...
    <title>Instagram - user photos</title>
  ...
  <body>
    <div class="container">
      <header class="clearfix">
        ...
        <h1>Instagram <span><?php echo $user; ?> Photos</span></h1>
      </header>
